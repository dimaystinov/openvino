...
int main() {
    ...
    for (size_t size : {0, 51, 131, 251, 263}) {
        auto b = bytes;
        b.resize(size);
        reject(b);
    }
    for (const auto& edit : std::vector<std::pair<size_t, uint32_t>>{
             {52, 1},  {60, 7},           {64, 1},           {56, 999},   {68, 0},  {68, 999},   {80, 0},
             {80, 4},  {116, 0},          {128, 319},        {140, 5000}, {160, 9}, {164, 0x11}, {168, 9},
             {172, 1}, {176, 0xffffffff}, {184, 0xffffffff}, {252, 0},    {260, 0}, {264, 1}}) {
        auto b = bytes;
        put(b, edit.first, edit.second);
        reject(b);
    }
    auto compact = bytes;
    put(compact, 264, 3);
