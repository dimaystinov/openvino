...
def main():
    core = ov.Core()
    core.register_plugin(sys.argv[1], 'MYRIAD_TEST')
    assert core.get_property('MYRIAD_TEST','AVAILABLE_DEVICES') == ['TEST-NOT-A-USB-DEVICE']
    compiled = core.import_model(io.BytesIO(blob()), 'MYRIAD_TEST', {})
    ...
    reloaded = core.import_model(exported,'MYRIAD_TEST',{})
    ...
    expect_error(lambda: core.compile_model(model,'MYRIAD_TEST'), '.blob only')
    expect_error(lambda: core.import_model(io.BytesIO(b'bad'),'MYRIAD_TEST',{}), 'invalid blob length')
    expect_error(lambda: core.set_property('MYRIAD_TEST',{'UNKNOWN':1}), 'unsupported property')
