...
inline std::mutex& transport_mutex() {
...
class Session {
    ...
    void close() noexcept {
        if (input)
            ncFifoDestroy(&input);
        if (output)
            ncFifoDestroy(&output);
        if (graph)
            ncGraphDestroy(&graph);
        if (device)
            ncDeviceClose(&device, watchdog);
        if (watchdog)
...
public:
    Session(const Blob& blob, const std::string& firmware, const std::string& id) {
        ...
        try {
            ...
            ncDeviceDescr_t description{};
            description.protocol = NC_USB;
            ...
            params.watchdogHndl = watchdog;
            params.watchdogInterval = 1000;
            ...
            check(ncGraphAllocate(device, graph, blob.data.data(), blob.file_size, blob.data.data(), Blob::header_size),
                  "upload graph");
            int count = 0;
            unsigned int length = sizeof(count);
    ...
    void infer(const std::vector<uint8_t>& in, std::vector<uint8_t>& out) {
        ...
        try {
            ...
            check(ncGraphQueueInferenceWithFifoElem(graph, input, output, in.data(), &size, nullptr),
                  "queue inference");
            require(size == in.size(), "partial input write");
            size = static_cast<unsigned int>(out.size());
            void* token = nullptr;
