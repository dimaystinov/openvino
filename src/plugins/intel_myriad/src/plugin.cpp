...
static ov::element::Type element(const Port& port) {
    static const ov::element::Type types[] = {ov::element::f16,
                                              ov::element::u8,
                                              ov::element::i32,
                                              ov::element::f32,
                                              ov::element::i8};
...
public:
    ...
    void validate_and_infer_types() override {
        ...
        for (size_t i = 0; i < ports.size(); ++i)
            set_output_type(i, element(ports[i]), ov::Shape(ports[i].shape));
    }
...
static std::shared_ptr<ov::Model> metadata_model(const Blob& blob) {
    ...
    for (const auto& port : blob.inputs) {
        ...
        node->set_friendly_name(port.name);
        node->output(0).set_names({port.name});
...
struct Config {
    ...
    Config() {
        if (auto* path = std::getenv("NCS2_FIRMWARE_DIR"))
            firmware = path;
    }
    void update(const ov::AnyMap& values) {
        for (const auto& pair : values) {
            if (pair.first == "NCS2_FIRMWARE_DIR")
                firmware = pair.second.as<std::string>();
            else if (pair.first == ov::device::id.name())
                id = pair.second.as<std::string>();
            else
                OPENVINO_THROW("NCS2: unsupported property ", pair.first);
        }
    }
};
class Request;
...
public:
    ...
    Compiled(std::shared_ptr<const Blob> b, const std::shared_ptr<const ov::IPlugin>& plugin, const Config& config)
        : ICompiledModel(metadata_model(*b), plugin),
          model(metadata_model(*b)),
          blob(b),
    ...
    ov::Any get_property(const std::string& name) const override {
        ...
        if (name == ov::execution_devices.name())
            return std::vector<std::string>{"MYRIAD"};
...
public:
    explicit Request(const std::shared_ptr<const Compiled>& model)
        : ISyncInferRequest(model),
          compiled(model),
          input(model->blob->input_size),
          output(model->blob->output_size) {
        for (const auto& port : get_inputs())
            allocate_tensor(port, [port](ov::SoPtr<ov::ITensor>& tensor) {
...
public:
    Plugin() {
        set_device_name("MYRIAD");
    }
    std::shared_ptr<ov::ICompiledModel> compile_model(const std::shared_ptr<const ov::Model>&,
                                                      const ov::AnyMap&) const override {
        OPENVINO_THROW(
            "NCS2: this plugin imports static MYRIAD 6.0 .blob only; compile IR using OpenVINO 2022.3 offline");
    }
    std::shared_ptr<ov::ICompiledModel> compile_model(const std::shared_ptr<const ov::Model>&,
                                                      const ov::AnyMap&,
    ...
    std::shared_ptr<ov::ICompiledModel> import_model(const ov::Tensor& tensor,
                                                     const ov::AnyMap& properties) const override {
        require(tensor.is_continuous() && tensor.get_element_type() == ov::element::u8 &&
                    tensor.get_byte_size() <= Blob::max_size,
                "expected compact u8 blob bytes");
    ...
    ov::Any get_property(const std::string& name, const ov::AnyMap&) const override {
        if (name == "INTERNAL_SUPPORTED_PROPERTIES")
        ...
        if (name == ov::supported_properties.name())
            return std::vector<ov::PropertyName>{ov::supported_properties,
                                                 ...
                                                 ov::device::id,
                                                 {"NCS2_FIRMWARE_DIR", ov::PropertyMutability::RW}};
        if (name == ov::available_devices.name())
            return devices();
        if (name == ov::device::full_name.name())
            return std::string("Intel Neural Compute Stick 2 (MYRIAD blob runtime)");
        if (name == ov::device::capabilities.name())
            return std::vector<std::string>{};
        std::lock_guard<std::mutex> lock(config_mutex);
        if (name == ov::device::id.name())
            return config.id;
        if (name == "NCS2_FIRMWARE_DIR")
...
static const ov::Version version = {CI_BUILD_NUMBER, "TEST ONLY: fake NCS2 USB transport"};
#else
static const ov::Version version = {CI_BUILD_NUMBER, "NCS2 native blob plugin"};
#endif
OV_DEFINE_PLUGIN_CREATE_FUNCTION(ncs2::Plugin, version)

// MYRIAD has one candidate library; no device-name dispatch is needed.
