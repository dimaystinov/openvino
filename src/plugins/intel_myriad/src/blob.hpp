...
inline void require(bool condition, const std::string& message) {
    if (!condition)
        throw std::runtime_error("NCS2: " + message);
}
struct Port {
    std::string name;
...
struct Blob {
    ...
    std::vector<Port> parse_ports(uint32_t count, size_t pos, size_t buffer_size, size_t constants) const {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            require(p.type < 5, "unsupported element type");
            p.item_size = sizes[p.type];
            ...
            for (uint32_t axis = 0; axis < rank; ++axis) {
                ...
                elements *= dim;
                span += (dim - 1) * stride;
                inner_span = dim * stride;
                p.shape.insert(p.shape.begin(), dim);
                p.strides.insert(p.strides.begin(), stride);
            }
            require(order == 0, "rank and order disagree");
            p.bytes = elements * p.item_size;
            p.span = span;
            p.compact = span == p.bytes;
...
inline void transfer(const Port& p, void* host, uint8_t* packed, bool to_device) {
    ...
    if (p.compact) {
        if (to_device)
            std::memcpy(packed, h, p.bytes);
        else
            std::memcpy(h, packed, p.bytes);
        return;
    }
    for (size_t index = 0; index < p.bytes / p.item_size; ++index) {
        ...
        if (to_device)
            std::memcpy(packed + offset, h + index * p.item_size, p.item_size);
        else
            std::memcpy(h + index * p.item_size, packed + offset, p.item_size);
    }
}
}  // namespace ncs2
